...
- [x] `usage_patterns` (scope `protocol_item`) conectado à geração real de protocolo: `/api/ai/generate` (task `generate-protocol`) agora carrega os 8 padrões de tarefa mais repetidos pela nutricionista (título/tipo/pontos) e injeta como referência de estilo no prompt antes de chamar a IA — antes o aprendizado só alimentava autocomplete manual na UI, nunca influenciava a geração por IA em si. `manager_learning` não foi conectado a esse fluxo de propósito: é o loop de feedback dos 10 agentes do orchestrator (chave `agent_name`+`action_type`, alimentado por `agent_pending_actions`), um mecanismo assíncrono e por agente — não existe hoje um "agente" de geração de protocolo nesse sentido, então bridging exigiria criar uma fila de aprovação nova para um fluxo que hoje é síncrono e sem fila; ficou fora de escopo
- [ ] `supplement_note` (sugestão de suplementação por tipo de refeição) só existe hardcoded em `lib/meal-plan-template.ts` (5 dos 10 slots: shot, ceia, pré/intra/pós-treino) — não é persistido no banco nem editável pela nutricionista na Biblioteca Clínica; hoje só serve o cardápio-modelo Fase 1 seedado em `20260727000002_seed_fase1_cardapio_base.sql`
- [ ] Tabela legada `club_plans` (`ClubPlanView.tsx`/`clubPlanActions.ts`, planejamento semestral em JSON blob) continua paralela ao novo `business_plans`/planejamento anual — decisão de migrar ou depreciar adiada deliberadamente (auditoria de Jul/2026, ver item de unificação faseada abaixo)
...
- [x] Camada **Meta** (`goals`) ganhou atribuição por paciente: nova tabela `patient_goal_assignments` (migration `20260803000001_patient_goal_assignments.sql`) guarda um snapshot da meta no momento da atribuição — mesmo racional de `protocol_assignments` vs `protocols`, editar o template depois não altera retroativamente a meta em andamento da paciente. Diferente de protocolo, a paciente pode ter várias metas ativas ao mesmo tempo, então não há "atual" único nem cancelamento automático ao atribuir outra. Admin: botão "Metas" no detalhe da paciente (`PatientsView.tsx`) abre modal de atribuir/remover, sobre `GET /api/admin/goals-list` (biblioteca) e `GET /api/admin/patients/[id]/goals` (atribuídas), com as ações `assign-goal`/`unassign-goal` na rota `[id]/action`. Paciente: nova tela `/patient/goals` (linkada em "Mais") lista metas ativas/concluídas via `GET /api/patient/goals`, com progresso registrado em `POST /api/patient/goals/progress` — completa automaticamente ao atingir `target_value` (ou no primeiro registro positivo, se a meta não tiver valor-alvo) e credita 50 XP via `awardPoints`. A sugestão por IA no formulário de criar meta (`GoalComponents.tsx`) trocou de `marketing-suggestion` (só preenchia descrição) para uma task dedicada `generate-goal` (`ai-security.ts` + `/api/ai/generate`), que agora também sugere tipo, métrica e valor-alvo a partir do título
- [ ] Auditoria de sistema (Jul/2026) identificou várias sobreposições de funcionalidade — progresso da unificação faseada:
  - [x] Cálculo de risco de paciente unificado em `lib/services/riskScore.ts`, consumido por `admin/patients`, `admin/segment-preview` e `admin/analytics`
  - [x] Escrita de XP centralizada em `lib/services/gamification.ts` (`awardPoints`); client não chama mais a RPC `increment_user_points` diretamente (movido para `POST /api/patient/protocol-progress`). O trigger `update_gamification_after_log` (daily_logs) continua sendo um mecanismo separado, fora de escopo
  - [x] Preço dos planos unificado: Settings é a única escrita; Billing virou leitura; Sales Page e a landing pública (`/vender/[slug]`) passaram a puxar o preço real de `/api/admin/checkout-plans` / `/api/tenant-info` em vez de texto livre
  - [x] Catálogo de produtos unificado na tabela `products` (migration `20260703000003_unify_product_catalog.sql`). `gateway_products` fica deprecated (não dropada ainda) e `/api/admin/gateway-products` + `/api/patient/gateway-products` viraram adaptadores sobre `products` via `lib/services/productCatalog.ts`, para não exigir reescrever `ProductGatewayView`/`/patient/gateway` nesta passada. **Atenção**: preço dos produtos migrados de `gateway_products` (texto livre tipo "R$ 297") NÃO foi convertido automaticamente para `price_cents` — ficou 0 com o texto original em `price_label_legacy`; precisa revisão manual na tela de Catálogo de Produtos antes de considerar esses itens com preço correto. Ainda falta decidir se as duas telas de admin (`ProductsView`/`ProductGatewayView`) serão fundidas numa só UI — hoje continuam separadas, só o dado é compartilhado
  - [x] Email Marketing fundida como aba "E-mail" dentro de `CommunicationCenterView` (agora com abas Push/Inbox + E-mail); view e entrada de menu antigas removidas. Régua de Eventos (Strategic Planner) **não** foi fundida de propósito — é um calendário editorial de conteúdo/protocolos/desafios, propósito diferente de "compor e enviar campanha agora" — ganhou só um link cruzado no rodapé da Central de Comunicação
  - [x] Investigado o par `patient_alarms`/`patient_reminders`: não são simplesmente duplicados — são dois mecanismos de entrega diferentes (alarms = push via OneSignal/cron, funciona com o app fechado; reminders = `Notification` API do navegador, só funciona com a aba aberta, via `ReminderSettings.tsx`). A rota `admin/cron/reminders`, que tentava dar a `patient_reminders` um disparo por push como o de `patient_alarms`, nunca era chamada por nada (não registrada em `vercel.json`) — removida por ser código genuinamente morto. `patient_reminders`/`ReminderSettings.tsx` continuam ativos como estão; decidir se vale a pena unificar as duas telas (Perfil → Lembretes vs Perfil → Alarmes) numa UX só fica para quando houver decisão de produto sobre isso
  - [x] Navegação da paciente reorganizada: bottom-nav vira Início/Dieta/Progresso/Tribo/Mais — Hábitos migrou pra dentro da Home, e "Mais" dá acesso a tudo que antes não tinha link nenhum (`/patient/recipes`, `/patient/scanner`, Diário Alimentar, Chat, Loja, Medidas, Alarmes, Inbox, Questionários, Consultas). A página `/patient/ranking` standalone continua existindo mas não foi religada — a aba Ranking dentro do Feed já cobre o caso de uso
  - [x] Check-in diário e semanal ganharam navegação cruzada: CTA do diário adicionado na Home ao lado do semanal, e link de um para o outro nas telas de conclusão de cada fluxo. Os nomes de campo divergentes entre as tabelas (`fase_ciclo` vs `cycle_phase`) não foram unificados — são tabelas com propósitos diferentes (diário granular vs. semanal com insight de IA) e renomear coluna em produção não valia o risco só por consistência de nome
- [x] Bug de contraste sistêmico corrigido (Ago/2026): o PR de reorganização de navegação/integrações (`docs/implementation/ux-integrations-rollout.md`, Onda 1) trocou os tokens globais (`app/globals.css`) e 5 arquivos de shell (`AdminClientPage.tsx`, `admin/settings/vital/page.tsx`, `patient/layout.tsx`, `BottomNav.tsx`) para um tema claro, mas nunca migrou as ~30 views do admin nem a maioria das páginas da paciente, que continuavam no tema escuro desta seção 4 — resultado: `text-white`/`bg-white/5`/`border-white/10` praticamente invisíveis sobre o novo fundo claro em quase todo o sistema e no app. Os 5 arquivos de shell voltaram à paleta escura aprovada (indigo/emerald/amber/rose sobre `bg-slate-950`); a reorganização da sidebar por domínio, a remoção dos atalhos flutuantes e o catálogo de integrações (que já excluía WebDiet de propósito) foram mantidos como estavam — só a paleta retrocedeu, não a estrutura. Não reintroduzir um tema claro global sem migrar todas as views na mesma leva

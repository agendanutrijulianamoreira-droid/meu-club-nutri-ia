...
- [ ] Auditoria de sistema (Jul/2026) identificou várias sobreposições de funcionalidade — progresso da unificação faseada:
  ...
  - [ ] Dois sistemas de lembrete paralelos e sobrepostos (`patient_alarms`, com cron de push ativo, e `patient_reminders`, com notificação client-side via `ReminderSettings.tsx` — o cron de push de `patient_reminders`, em `/api/admin/cron/reminders`, está órfão e nunca é chamado)

...
function buildTrend(label: string, currentValues: Array<number | null | undefined>, previousValues: Array<number | null | undefined>, lowerIsBetter: boolean): Trend | null {
  ...
  return { label, direction: improved ? "better" : "worse", text: improved ? (lowerIsBetter ? "reduzindo" : "melhorando") : (lowerIsBetter ? "aumentando" : "reduzindo") }
}
  ...
  return (
    <section className="bg-background text-[#2B1A10]">
      <div className="max-w-[460px] mx-auto px-4 pb-28">
        <div className="mb-3"><p className="text-[10px] uppercase tracking-[0.2em] font-bold text-[#C9A435]">Últimos 7 dias</p><h2 className="font-serif text-xl font-semibold mt-0.5">Sua evolução</h2></div>
        <div className="rounded-3xl bg-white border border-[#2B1A10]/10 p-5 shadow-sm">
          <div className="flex items-center justify-between gap-4 mb-4"><div><p className="text-sm font-semibold">Consistência da semana</p><p className="text-xs text-[#2B1A10]/45 mt-0.5">{data.activeDays} de 7 dias com algum registro</p></div><div className="h-12 w-12 rounded-2xl bg-[#C9A435]/10 flex items-center justify-center text-[#9B7A16] font-bold text-sm">{consistency}%</div></div>
          <div className="h-2 rounded-full bg-[#2B1A10]/5 overflow-hidden mb-5"><div className="h-full rounded-full bg-[#C9A435]" style={{ width: `${consistency}%` }} /></div>
          <div className="grid grid-cols-3 gap-2 mb-5">
            <div className="rounded-2xl bg-[#F4EFE4]/70 p-3 text-center"><HeartPulse size={17} className="mx-auto text-[#C9A435] mb-1" /><p className="text-lg font-bold">{data.checkinsThisWeek}</p><p className="text-[10px] text-[#2B1A10]/45">check-ins</p></div>
            <div className="rounded-2xl bg-[#F4EFE4]/70 p-3 text-center"><Droplet size={17} className="mx-auto text-[#C9A435] mb-1" /><p className="text-lg font-bold">{data.hydrationDays}/7</p><p className="text-[10px] text-[#2B1A10]/45">meta de água</p></div>
            <div className="rounded-2xl bg-[#F4EFE4]/70 p-3 text-center"><CheckCircle2 size={17} className="mx-auto text-[#C9A435] mb-1" /><p className="text-lg font-bold">{data.missionsCompleted}</p><p className="text-[10px] text-[#2B1A10]/45">missões feitas</p></div>
          </div>
          <div className="border-t border-[#2B1A10]/5 pt-4">
            <div className="flex items-center gap-2 mb-3"><Activity size={15} className="text-[#C9A435]" /><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#2B1A10]/55">Sinais percebidos</p></div>
            {data.trends.length ? <div className="space-y-2.5">{data.trends.map(trend => { const Icon = trend.direction === "better" ? TrendingUp : trend.direction === "worse" ? TrendingDown : Minus; return <div key={trend.label} className="flex items-center justify-between gap-3"><span className="text-sm font-medium">{trend.label}</span><span className={`text-xs font-bold flex items-center gap-1 ${trend.direction === "better" ? "text-emerald-700" : trend.direction === "worse" ? "text-amber-700" : "text-[#2B1A10]/45"}`}><Icon size={13} /> {trend.text}</span></div> })}</div> : <div className="rounded-2xl bg-[#F4EFE4]/60 px-4 py-3 flex items-start gap-2.5"><Sparkles size={15} className="text-[#C9A435] mt-0.5 shrink-0" /><p className="text-xs text-[#2B1A10]/55 leading-relaxed">Continue fazendo seus check-ins. Quando houver dados suficientes de duas semanas, mostramos tendências reais.</p></div>}
          </div>
          <Link href="/patient/progresso" className="mt-4 w-full rounded-2xl border border-[#2B1A10]/10 py-3 text-xs font-bold flex items-center justify-center hover:bg-[#F4EFE4]/50 transition">Ver evolução completa</Link>
        </div>
      </div>
    </section>
  )

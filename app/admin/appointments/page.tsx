...
export default async function AppointmentsPage({searchParams}:{searchParams?:{q?:string;status?:string;saved?:string;error?:string}}){
  const{supabase,tenantId}=await viewer();const q=String(searchParams?.q||'').trim().slice(0,80);const status=String(searchParams?.status||'active')
  ...
  let contactQuery=supabase.from('crm_contacts').select('id,name,email,phone,whatsapp,linked_user_id').eq('tenant_id',tenantId).eq('do_not_contact',false).order('name').limit(q?50:25)
  ...
  let appointmentsQuery=supabase.from('appointments').select('id,scheduled_at,ends_at,blocked_ends_at,duration_minutes,status,is_virtual,meeting_link,location_address,notes,schedule_override,override_reason,crm_contact:crm_contacts!crm_contact_id(id,name,email,phone,whatsapp),appointment_type:appointment_types!appointment_type_id(id,name,code),nutritionist:nutritionists!nutritionist_id(id,name)').eq('tenant_id',tenantId).gte('scheduled_at',from).lte('scheduled_at',to).order('scheduled_at',{ascending:true}).limit(500)
  ...
  const{data:appointments,error:loadError}=await appointmentsQuery
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-black">Nova consulta</h2><p className="mt-1 text-sm text-slate-600">Pesquise o contato no CRM. Ele não precisa ter login no app.</p>
      <form method="get" className="mt-4 flex gap-2"><input name="q" defaultValue={q} placeholder="Buscar contato pelo nome" className="min-w-0 flex-1 rounded-xl border border-slate-300 px-3 py-2"/><button className="rounded-xl border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-bold">Buscar</button></form>

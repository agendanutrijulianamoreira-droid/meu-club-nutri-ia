...
export default function AdminDashboard({ userName = 'Admin', tenantName = '', role = 'admin', tenantId = '', needsRepair = false }: AdminDashboardProps) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white flex">
            <aside style={{ width: sidebarWidth }} className="fixed inset-y-0 left-0 z-50 flex flex-col bg-slate-950 border-r border-white/10 transition-[width] duration-200">
                <div className={`h-20 flex items-center border-b border-white/10 ${collapsed ? 'justify-center px-2' : 'justify-between px-5'}`}>
                    <button onClick={() => navigate('dashboard')} className="flex items-center gap-3 min-w-0" title="Ir para o painel">
                        <div className="h-10 w-10 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center shrink-0"><Brain size={20} className="text-indigo-400" /></div>
                        {!collapsed && <div className="text-left min-w-0"><p className="text-[15px] font-black leading-tight text-white truncate">{tenantName || 'NutriOS'}</p><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500 mt-0.5">Gestão clínica</p></div>}
                    </button>
                    {!collapsed && <button onClick={() => setCollapsed(true)} className="h-9 w-9 rounded-xl flex items-center justify-center text-slate-400 hover:text-indigo-400 hover:bg-white/5" aria-label="Recolher menu"><PanelLeftClose size={18} /></button>}
                </div>
                {collapsed && <button onClick={() => setCollapsed(false)} className="mx-auto mt-3 h-9 w-9 rounded-xl flex items-center justify-center text-slate-400 hover:text-indigo-400 hover:bg-white/5" aria-label="Expandir menu"><PanelLeftOpen size={18} /></button>}
                <nav className="flex-1 overflow-y-auto custom-scrollbar px-3 py-3 space-y-1">
                    {navGroups.map(group => {
                        ...
                        return <div key={group.id}>
                            <button onClick={() => toggleGroup(group.id)} title={collapsed ? group.label : undefined} className={`w-full flex items-center rounded-xl transition-colors ${collapsed ? 'justify-center h-11' : 'gap-3 px-3 py-2.5'} ${groupActive ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
                                <div className="relative shrink-0"><Icon size={18} strokeWidth={1.8} />{groupBadge && collapsed && <span className="absolute -top-1.5 -right-1.5 h-2.5 w-2.5 rounded-full bg-rose-500 border-2 border-slate-950" />}</div>
                                {!collapsed && <><span className="flex-1 text-left text-[13px] font-bold">{group.label}</span>{groupBadge && <span className="min-w-[20px] h-5 px-1 rounded-full bg-rose-500 text-white text-[10px] font-black flex items-center justify-center">{pendingApprovals > 9 ? '9+' : pendingApprovals}</span>}<ChevronRight size={14} className={`transition-transform ${expanded ? 'rotate-90' : ''}`} /></>}
                            </button>
                            <AnimatePresence initial={false}>{expanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="ml-6 pl-3 border-l border-white/10 py-1 space-y-0.5">
                                {group.items.map(item => {
                                    const active = item.id ? activeView === item.id : false
                                    const cls = `w-full flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-left text-[12px] font-semibold transition-colors ${active ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`
                                    const content = <><span>{item.label}</span>{item.badge && pendingApprovals > 0 && <span className="min-w-[18px] h-[18px] px-1 rounded-full bg-rose-500 text-white text-[9px] font-black flex items-center justify-center">{pendingApprovals > 9 ? '9+' : pendingApprovals}</span>}</>
                ...
                {!collapsed && <div className="border-t border-white/10 p-3"><Link href="/admin/settings/vital" className="flex items-center gap-3 rounded-xl border border-indigo-500/30 bg-indigo-500/10 px-3 py-3 text-indigo-400 hover:bg-indigo-500/15 transition-colors"><SlidersHorizontal size={17} /><div className="min-w-0"><p className="text-xs font-black">Chaves e integrações</p><p className="text-[10px] text-slate-500 mt-0.5">Central de serviços vitais</p></div></Link></div>}
            </aside>
            <main style={{ marginLeft: sidebarWidth }} className="flex-1 flex flex-col min-h-screen transition-[margin] duration-200 min-w-0">
                <header className="h-20 px-5 md:px-8 flex items-center justify-between bg-slate-950/90 backdrop-blur border-b border-white/10 sticky top-0 z-30">
                    <div className="min-w-0"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500"><ShieldCheck size={13} className="text-emerald-400" />Operação da clínica</div><p className="mt-1 text-sm font-bold text-white truncate">{navGroups.find(g => g.id === activeGroupId)?.label || 'Painel'}</p></div>
                    <div className="flex items-center gap-3"><Link href="/admin/attention" className="hidden lg:flex items-center gap-2 rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs font-bold text-amber-400 hover:bg-amber-500/15"><HeartPulse size={15} /> Prioridades do dia</Link><UserDropdown userName={userName} role={role} openOverlay={openOverlay} router={router} /></div>
                </header>
...
function UserDropdown({ userName, role, openOverlay, router }: { userName: string; role: string; openOverlay: (o: any) => void; router: ReturnType<typeof useRouter> }) {
    ...
    return <div className="relative">
        <button className="flex items-center gap-3 rounded-xl p-1.5 hover:bg-white/5 transition-colors" onClick={() => setIsOpen(v => !v)}><div className="hidden sm:block text-right"><p className="text-xs font-bold text-white leading-none">{userName}</p><p className="text-[10px] text-slate-500 font-semibold mt-1">{role === 'admin' ? 'Administradora' : 'Nutricionista'}</p></div><div className="h-9 w-9 rounded-xl border border-white/10 overflow-hidden bg-indigo-500/10"><img src={`https://api.dicebear.com/9.x/micah/svg?seed=${userName}`} className="w-full h-full" alt="Perfil" /></div><ChevronDown size={13} className={`text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} /></button>
        <AnimatePresence>{isOpen && <><div className="fixed inset-0 z-40" onClick={() => setIsOpen(false)} /><motion.div initial={{ opacity: 0, y: 8, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 6, scale: .98 }} className="absolute right-0 mt-2 w-60 bg-slate-900 border border-white/10 rounded-2xl shadow-xl p-2 z-50">{menuItems.map(item => <button key={item.id} onClick={() => { item.onClick(); setIsOpen(false) }} className="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-white hover:bg-white/5 rounded-xl transition-colors"><item.icon size={16} /><span className="text-xs font-semibold">{item.label}</span></button>)}<Link href="/admin/settings/vital" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-indigo-400 hover:bg-indigo-500/10 rounded-xl transition-colors"><Settings size={16} /><span className="text-xs font-semibold">Chaves e integrações</span></Link><div className="h-px bg-white/10 my-1 mx-2" /><button onClick={handleSignOut} className="w-full flex items-center gap-3 px-4 py-3 text-rose-400 hover:bg-rose-500/10 rounded-xl transition-colors"><LogOut size={16} /><span className="text-xs font-semibold">Sair do sistema</span></button></motion.div></>}</AnimatePresence>
    </div>
}

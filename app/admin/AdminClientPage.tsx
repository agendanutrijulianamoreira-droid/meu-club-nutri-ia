    ...
    return (
        <div className="theme-admin-light min-h-screen bg-[#F4F7F6] text-[#1C2B27] flex">
            ...
            <main style={{ marginLeft: undefined }} className={`flex-1 flex flex-col min-h-screen min-w-0 md:transition-[margin] md:duration-200 ${collapsed ? 'md:ml-[76px]' : 'md:ml-[272px]'}`}>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar"><AnimatePresence mode="wait"><motion.div key={activeView} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.16 }} className="legacy-admin-light-bridge p-4 md:p-7 xl:p-8 max-w-[1500px] mx-auto w-full">{renderView()}</motion.div></AnimatePresence></div>
            </main>
        </div>
    )
}

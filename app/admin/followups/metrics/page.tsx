...
export default async function FollowupMetricsPage() {
  ...
  const interventionTimes = completed
    .map((t:any) => t.executed_at ? new Date(t.executed_at).getTime() - new Date(t.created_at).getTime() : null)
  ...
  const feedback = pending.filter((t:any)=>t.action_type==='weekly_checkin_feedback').length

  const cards = [
    ['Tarefas criadas', all.length, `Últimos ${windowDays} dias`],
    ['Pendentes agora', pending.length, 'Fila ainda aberta'],
    ['Concluídas', completed.length, `${pct(completed.length,all.length)}% das criadas`],
    ['Dispensadas', dismissed.length, `${pct(dismissed.length,all.length)}% das criadas`],
    ['Tempo médio até intervenção', avgHours === null ? '—' : `${avgHours} h`, 'Da criação até conclusão'],
  ...
  return <main className="min-h-screen bg-[#090B10] px-4 py-8 text-slate-100"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <div className="rounded-2xl border border-rose-400/15 bg-rose-400/5 p-4"><p className="text-xs text-rose-200">Críticas hoje</p><p className="mt-1 text-2xl font-black">{critical}</p></div>
      <div className="rounded-2xl border border-amber-400/15 bg-amber-400/5 p-4"><p className="text-xs text-amber-200">Para hoje</p><p className="mt-1 text-2xl font-black">{todayCount}</p></div>
      <div className="rounded-2xl border border-sky-400/15 bg-sky-400/5 p-4"><p className="text-xs text-sky-200">Em reativação</p><p className="mt-1 text-2xl font-black">{reactivation}</p></div>
      <div className="rounded-2xl border border-violet-400/15 bg-violet-400/5 p-4"><p className="text-xs text-violet-200">Feedbacks pendentes</p><p className="mt-1 text-2xl font-black">{feedback}</p></div>
    </section>

    <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-4 text-xs leading-relaxed text-slate-500">“Recuperação após intervenção” considera pacientes com tarefa <code>human_followup</code> concluída dentro da janela e cujo snapshot atual não está mais em <code>inactive</code> ou <code>at_risk</code>. A janela é editável no Admin.</div>
  </div></main>
}

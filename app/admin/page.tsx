        ...
        <>
            ...
            {props.tenantId && (
                <div className="fixed bottom-5 right-5 z-[80] flex flex-col items-end gap-2">
                    <Link href="/admin/appointments" className="inline-flex items-center gap-2 rounded-2xl border border-violet-300/40 bg-violet-600 px-4 py-3 text-xs font-black text-white shadow-2xl shadow-black/30 transition hover:bg-violet-500" aria-label="Abrir agenda operacional"><CalendarClock size={17} />Agenda operacional</Link>
...
export default async function AdminPage() {
    ...
    if (isAdmin && (!profile?.tenant_id || isDemoTenant)) {
        ...
        if (ownedTenants && ownedTenants.length > 0) {
            return <AdminDashboardWithAttention userName={userName} tenantName={ownedTenants[0].brand_name || ''} role="admin" tenantId={ownedTenants[0].id} needsRepair={true} />;
        }
        redirect('/admin/clinic');
    }

    if (session && !isAdmin) redirect('/patient/home');
    if (session && isAdmin && profile) return <AdminDashboardWithAttention {...dashboardProps} />;
    redirect('/login');
}

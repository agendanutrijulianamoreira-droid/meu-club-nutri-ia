...
async function saveSettings(form:FormData){'use server'
  ...
  const{error}=await supabase.from('tenant_appointment_settings').upsert({
    tenant_id:tenantId,
    timezone,
    no_show_enabled:form.get('no_show_enabled')==='on',
    no_show_grace_minutes:grace,
    allow_staff_schedule_override:form.get('allow_staff_schedule_override')==='on',
    require_cancellation_reason:form.get('require_cancellation_reason')==='on',
    allow_custom_duration:form.get('allow_custom_duration')==='on',
    updated_by:user.id,
...
export default async function AppointmentSettingsPage({searchParams}:{searchParams?:{saved?:string;typeSaved?:string;error?:string}}){
  ...
  const[{data:settings},{data:types}]=await Promise.all([
    supabase.from('tenant_appointment_settings').select('timezone,no_show_enabled,no_show_grace_minutes,allow_staff_schedule_override,require_cancellation_reason,allow_custom_duration').eq('tenant_id',tenantId).maybeSingle(),
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900"><div className="mx-auto max-w-5xl space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.2em] text-amber-700">Fase 4 · Automação da consulta</p><h1 className="text-3xl font-black">Configurações de consulta</h1><p className="mt-2 max-w-3xl text-sm text-slate-600">Regras da clínica que podem mudar sem alterar código: fuso, ausência, encaixes, cancelamentos, duração e tipos de consulta.</p></div><div className="flex gap-2"><Link href="/admin/appointments" className="rounded-xl bg-violet-700 px-4 py-2 text-sm font-bold text-white">Agenda operacional</Link><Link href="/admin" className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold">Voltar ao painel</Link></div></header>
    ...
    <form action={saveSettings} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-black">Agenda da clínica</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="grid gap-1 text-sm font-bold"><span>Timezone da clínica</span><input name="timezone" required defaultValue={settings?.timezone||'America/Sao_Paulo'} placeholder="America/Sao_Paulo" className="rounded-xl border border-slate-300 px-3 py-2"/><small className="font-normal text-slate-500">Use um timezone IANA. Todos os horários da consulta usam esta referência.</small></label><label className="grid gap-1 text-sm font-bold"><span>Minutos de tolerância antes de marcar ausência</span><input name="no_show_grace_minutes" type="number" min={0} max={1440} required defaultValue={settings?.no_show_grace_minutes??15} className="rounded-xl border border-slate-300 px-3 py-2"/></label></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="flex items-center gap-2 text-sm font-bold"><input name="no_show_enabled" type="checkbox" defaultChecked={settings?.no_show_enabled??true}/>Marcar ausência automaticamente após a tolerância</label><label className="flex items-center gap-2 text-sm font-bold"><input name="allow_staff_schedule_override" type="checkbox" defaultChecked={settings?.allow_staff_schedule_override??true}/>Permitir encaixe fora da disponibilidade</label><label className="flex items-center gap-2 text-sm font-bold"><input name="require_cancellation_reason" type="checkbox" defaultChecked={settings?.require_cancellation_reason??true}/>Exigir motivo ao cancelar consulta</label><label className="flex items-center gap-2 text-sm font-bold"><input name="allow_custom_duration" type="checkbox" defaultChecked={settings?.allow_custom_duration??true}/>Permitir duração diferente do tipo de consulta</label></div><button className="mt-5 rounded-xl bg-slate-900 px-4 py-3 text-sm font-black text-white">Salvar agenda da clínica</button></form>

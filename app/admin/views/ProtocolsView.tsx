...
export function ProtocolsView({ setView }: { setView: (v: any) => void }) {
    ...
    const { protocols, loading, createProtocol, updateProtocol, deleteProtocol, duplicateProtocol, refresh } = useProtocols()
...
function CreateProtocolForm({ onClose, onSave, onUpdate, editingData }: {
    ...
    const updateDayItem = (dayIndex: number, taskIndex: number, field: string, value: any) => {
        ...
        newDays[dayIndex].items[taskIndex][field] = value
    ...
    const addBlock = (dayIndex: number) => {
        ...
        newDays[dayIndex].items.push({ title: "", type: "custom", points: 10 })
        setFormData({ ...formData, days: newDays })
    }

    const removeBlock = (dayIndex: number, taskIndex: number) => {
        const newDays = [...formData.days]
        newDays[dayIndex].items.splice(taskIndex, 1)
        setFormData({ ...formData, days: newDays })
    }
                    ...
                    <div className="col-span-4 bg-white/5 rounded-xl p-4 overflow-y-auto space-y-2">
                        {formData.days.map((day: any, idx: number) => (
                            ...
                            >
                                ...
                                <p className="text-xs text-gray-400 mt-1">{day.items.length} tarefas</p>
                            </button>
                        ))}
                    </div>
                                        ...
                                        {formData.days[selectedDay].items.map((task: any, taskIdx: number) => (
                                            <div key={taskIdx} className="flex gap-2 items-start bg-white/5 p-3 rounded-lg">
                                                ...
                                                <input
                                                    ...
                                                    value={task.points}
                                                    onChange={e => updateDayItem(selectedDay, taskIdx, 'points', parseInt(e.target.value))}
                                                />
                                                <button
                                                    onClick={() => removeBlock(selectedDay, taskIdx)}

-- Fase 3 audit hardening: tenant link integrity, immutable metric semantics, configurable App->CRM mapping.
...
set outcome_code_snapshot = coalesce(o.outcome_code_snapshot,t.code),
    outcome_name_snapshot = coalesce(o.outcome_name_snapshot,t.name),
    counts_as_response_snapshot = coalesce(o.counts_as_response_snapshot,t.counts_as_response),
    counts_as_conversion_snapshot = coalesce(o.counts_as_conversion_snapshot,t.counts_as_conversion),
    owner_name_snapshot = coalesce(o.owner_name_snapshot,p.display_name,p.name),
    stage_name_snapshot = coalesce(o.stage_name_snapshot,s.name)
from public.crm_outcome_types t
left join public.profiles p on p.user_id=o.owner_user_id_snapshot and p.tenant_id=o.tenant_id
left join public.crm_stages s on s.id=o.stage_id_snapshot and s.tenant_id=o.tenant_id
where t.id=o.outcome_type_id and t.tenant_id=o.tenant_id;
...
create policy "Staff reads CRM stage mapping rules" on public.crm_stage_mapping_rules for select to authenticated
using (exists(select 1 from public.profiles p where p.user_id=(select auth.uid()) and p.tenant_id=crm_stage_mapping_rules.tenant_id and lower(coalesce(p.role,'')) in ('admin','nutritionist','nutri')));
create policy "Staff manages CRM stage mapping rules" on public.crm_stage_mapping_rules for all to authenticated
using (exists(select 1 from public.profiles p where p.user_id=(select auth.uid()) and p.tenant_id=crm_stage_mapping_rules.tenant_id and lower(coalesce(p.role,'')) in ('admin','nutritionist','nutri')))
with check (exists(select 1 from public.profiles p where p.user_id=(select auth.uid()) and p.tenant_id=crm_stage_mapping_rules.tenant_id and lower(coalesce(p.role,'')) in ('admin','nutritionist','nutri')));

create or replace function public.validate_crm_tenant_links()
...
drop trigger if exists trg_validate_crm_contacts_tenant_links on public.crm_contacts;
create trigger trg_validate_crm_contacts_tenant_links before insert or update on public.crm_contacts for each row execute function public.validate_crm_tenant_links();
drop trigger if exists trg_validate_crm_outcome_types_tenant_links on public.crm_outcome_types;
create trigger trg_validate_crm_outcome_types_tenant_links before insert or update on public.crm_outcome_types for each row execute function public.validate_crm_tenant_links();
drop trigger if exists trg_validate_crm_contact_events_tenant_links on public.crm_contact_events;
create trigger trg_validate_crm_contact_events_tenant_links before insert or update on public.crm_contact_events for each row execute function public.validate_crm_tenant_links();
drop trigger if exists trg_validate_crm_import_rows_tenant_links on public.crm_import_rows;
create trigger trg_validate_crm_import_rows_tenant_links before insert or update on public.crm_import_rows for each row execute function public.validate_crm_tenant_links();
drop trigger if exists trg_validate_crm_contact_outcomes_tenant_links on public.crm_contact_outcomes;
create trigger trg_validate_crm_contact_outcomes_tenant_links before insert or update on public.crm_contact_outcomes for each row execute function public.validate_crm_tenant_links();
drop trigger if exists trg_validate_crm_stage_mapping_tenant_links on public.crm_stage_mapping_rules;
create trigger trg_validate_crm_stage_mapping_tenant_links before insert or update on public.crm_stage_mapping_rules for each row execute function public.validate_crm_tenant_links();

-- Staff-created events must identify the signed-in actor.
...
with check (actor_user_id=(select auth.uid()) and exists(select 1 from public.profiles p where p.user_id=(select auth.uid()) and p.tenant_id=crm_contact_events.tenant_id and lower(coalesce(p.role,'')) in ('admin','nutritionist','nutri')));

create or replace function public.ensure_default_crm_stage_mapping_rules(p_tenant_id uuid)
...
begin
  ...
  select p_tenant_id,x.kind,x.value,s.id,x.priority
  from (values
    ('lifecycle','reactivation','reactivation',10),
    ('lifecycle','care_completed','care_completed',10),
    ('operational','inactive','attention',20),
    ('operational','at_risk','attention',20),
    ('operational','oscillating','attention',20),
    ('default','*','active_patient',999)
  ) as x(kind,value,stage_code,priority)
  join public.crm_stages s on s.tenant_id=p_tenant_id and s.code=x.stage_code
  on conflict(tenant_id,source_kind,source_value) do nothing;
  get diagnostics v_rows=row_count;
  return v_rows;
end $$;
revoke all on function public.ensure_default_crm_stage_mapping_rules(uuid) from public,anon,authenticated;
grant execute on function public.ensure_default_crm_stage_mapping_rules(uuid) to service_role;

select public.ensure_default_crm_stage_mapping_rules(id) from public.tenants;

create or replace function public.resolve_crm_stage_id(p_tenant_id uuid,p_lifecycle_status text,p_operational_status text)
returns uuid language sql stable security definer set search_path=public as $$
  select r.stage_id
  from public.crm_stage_mapping_rules r
  join public.crm_stages s on s.id=r.stage_id and s.tenant_id=r.tenant_id and s.active
  where r.tenant_id=p_tenant_id and r.active and (
    (r.source_kind='lifecycle' and r.source_value=coalesce(p_lifecycle_status,'')) or
    (r.source_kind='operational' and r.source_value=coalesce(p_operational_status,'')) or
    (r.source_kind='default' and r.source_value='*')
  )
  order by r.priority asc,
    case r.source_kind when 'lifecycle' then 0 when 'operational' then 1 else 2 end asc
  limit 1
$$;
revoke all on function public.resolve_crm_stage_id(uuid,text,text) from public,anon,authenticated;
grant execute on function public.resolve_crm_stage_id(uuid,text,text) to service_role;

create or replace function public.capture_crm_outcome_snapshot()
returns trigger language plpgsql set search_path=public as $$
declare v_contact public.crm_contacts%rowtype; v_type public.crm_outcome_types%rowtype; v_owner text; v_stage text;
begin
  select * into v_contact from public.crm_contacts where id=new.contact_id and tenant_id=new.tenant_id;
  ...
  select coalesce(p.display_name,p.name) into v_owner from public.profiles p where p.user_id=v_contact.owner_user_id and p.tenant_id=new.tenant_id;
  select s.name into v_stage from public.crm_stages s where s.id=v_contact.stage_id and s.tenant_id=new.tenant_id;
  ...
  new.stage_name_snapshot:=v_stage;
  new.outcome_code_snapshot:=v_type.code;
  new.outcome_name_snapshot:=v_type.name;
  new.counts_as_response_snapshot:=v_type.counts_as_response;
  new.counts_as_conversion_snapshot:=v_type.counts_as_conversion;
...
begin
  ...
  select coalesce(nullif(default_country_code,''),'55') into v_country from public.crm_import_settings where tenant_id=p_tenant_id;v_country:=coalesce(v_country,'55');
  perform public.ensure_default_crm_stage_mapping_rules(p_tenant_id);

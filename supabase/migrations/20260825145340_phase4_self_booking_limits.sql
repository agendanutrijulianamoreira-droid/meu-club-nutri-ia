alter table public.tenant_appointment_settings
  add column if not exists patient_self_booking_max_future integer not null default 1
  check (patient_self_booking_max_future between 1 and 20);
...
begin
  ...
  select count(*) into v_active_count from public.appointments a where a.tenant_id=v_tenant and a.patient_id=v_uid and a.status in ('scheduled','confirmed','in_progress') and a.scheduled_at>=now();
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('patient:'||v_uid::text,0));
  select count(*) into v_active_count from public.appointments a where a.tenant_id=v_tenant and a.patient_id=v_uid and a.status in ('scheduled','confirmed','in_progress') and a.scheduled_at>=now();
  if v_active_count>=coalesce(v_settings.patient_self_booking_max_future,1) then raise exception 'Você já atingiu o limite de consultas futuras que podem ser autoagendadas'; end if;
  ...
  values(v_tenant,v_contact_id,v_uid,p_nutritionist_id,p_appointment_type_id,v_start,v_type.duration_minutes,v_type.default_is_virtual,case when v_type.default_is_virtual then v_avail.default_meeting_link else null end,'scheduled',false,null,'Agendada pela paciente no app') returning id into v_id;
  return v_id;
end;$$;

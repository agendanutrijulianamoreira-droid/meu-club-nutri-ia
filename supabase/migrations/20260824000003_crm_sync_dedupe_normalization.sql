-- Phase 3: normalize app contacts and link existing CRM records before inserting duplicates.
...
BEGIN
  ...
  WITH latest_risk AS (
    ...
    UPDATE public.crm_contacts c SET linked_user_id=x.user_id,metadata=COALESCE(c.metadata,'{}'::jsonb)||jsonb_build_object('linked_from','app'),updated_at=now()
...
BEGIN
  ...
  UPDATE public.crm_contacts c SET recency_segment=CASE
    WHEN GREATEST(c.last_activity_at,c.last_consultation_at) IS NULL THEN 'unknown'
    WHEN p_reference_date-(GREATEST(c.last_activity_at,c.last_consultation_at) AT TIME ZONE 'America/Sao_Paulo')::date<=v_recent THEN 'recent'
    WHEN p_reference_date-(GREATEST(c.last_activity_at,c.last_consultation_at) AT TIME ZONE 'America/Sao_Paulo')::date<=v_warm THEN 'warm'
    WHEN p_reference_date-(GREATEST(c.last_activity_at,c.last_consultation_at) AT TIME ZONE 'America/Sao_Paulo')::date<=v_cold THEN 'cold'

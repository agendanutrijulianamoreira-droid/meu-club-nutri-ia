-- Fase 2 — corrige refresh do snapshot para respeitar calculated_date gerada.
-- calculated_date é GENERATED ALWAYS a partir de calculated_at, portanto não deve receber INSERT explícito.
...
BEGIN
  ...
  WITH patients AS (
    SELECT p.user_id, p.tenant_id, p.created_at::date AS created_date,
      COALESCE(p.onboarding_completed, false) AS onboarding_completed,
      p.plan_expires_at, COALESCE(p.current_streak, 0) AS current_streak
    FROM public.profiles p
    WHERE p.tenant_id = p_tenant_id AND p.role = 'patient'
  ),
  activity_dates AS (
    SELECT dl.user_id, dl.log_date AS activity_date
    FROM public.daily_logs dl JOIN patients p ON p.user_id = dl.user_id
    WHERE dl.log_date BETWEEN p_reference_date - 30 AND p_reference_date
    UNION
    SELECT cd.paciente_id, cd.data
    FROM public.checkin_diario cd JOIN patients p ON p.user_id = cd.paciente_id
    WHERE cd.data BETWEEN p_reference_date - 30 AND p_reference_date
    UNION
    SELECT wr.user_id, (wr.created_at AT TIME ZONE 'America/Sao_Paulo')::date
    FROM public.weekly_checkin_responses wr JOIN patients p ON p.user_id = wr.user_id
    WHERE (wr.created_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN p_reference_date - 30 AND p_reference_date
    UNION
    SELECT pa.user_id, pp.checkin_date
  ...
  activity AS (
    SELECT p.user_id, max(a.activity_date) AS last_activity_date,
    ...
    FROM patients p LEFT JOIN activity_dates a ON a.user_id = p.user_id
    GROUP BY p.user_id
  ),
  weekly AS (
    SELECT p.user_id,
      max((wr.created_at AT TIME ZONE 'America/Sao_Paulo')::date) AS last_weekly_checkin_date
    FROM patients p LEFT JOIN public.weekly_checkin_responses wr ON wr.user_id = p.user_id
    GROUP BY p.user_id
  ),
  appointments AS (
    SELECT p.user_id,
      ...
      max(COALESCE(a.completed_at, a.scheduled_at)) FILTER (WHERE a.status = 'completed') AS last_appointment_at,
      bool_or(
        a.scheduled_at < (p_reference_date::timestamp AT TIME ZONE 'America/Sao_Paulo')
        AND a.status IN ('scheduled', 'confirmed')
      ) AS consultation_overdue
    FROM patients p
    LEFT JOIN public.appointments a ON a.patient_id = p.user_id AND a.tenant_id = p.tenant_id
    GROUP BY p.user_id
  ),
  protocols AS (
    SELECT p.user_id,
      min(pa.end_date) FILTER (WHERE pa.status = 'active' AND pa.end_date >= p_reference_date) AS active_protocol_end_date
    FROM patients p LEFT JOIN public.protocol_assignments pa ON pa.user_id = p.user_id
    GROUP BY p.user_id
  ),
  base AS (
    SELECT p.*, a.last_activity_date, COALESCE(a.active_days_7d, 0) AS active_days_7d,
      ...
      w.last_weekly_checkin_date, ap.next_appointment_at, ap.last_appointment_at,
  ...
  scored AS (
    SELECT b.*,
      ...
      (b.onboarding_completed AND (
        b.last_weekly_checkin_date IS NULL OR p_reference_date - b.last_weekly_checkin_date >= 8
      )) AS checkin_overdue_calc,
      (b.plan_expires_at IS NOT NULL AND
        (b.plan_expires_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN p_reference_date AND p_reference_date + 15
      ) AS plan_expiring_calc,
      (b.active_protocol_end_date IS NOT NULL AND
        b.active_protocol_end_date BETWEEN p_reference_date AND p_reference_date + 7
  ...
  INSERT INTO public.patient_risk_scores (
    tenant_id, user_id, overall_risk, inactivity_risk, adherence_risk,
    emotional_risk, engagement_risk, risk_level, signals, recommended_action,
    action_taken, days_since_activity, current_streak, adherence_7d,
    last_checkin_score, calculated_at, operational_status, attention_bucket,
    last_activity_date, eligible_activity_days, next_appointment_at,
    last_appointment_at, active_protocol_end_date, checkin_overdue,
    consultation_overdue, plan_expiring, protocol_ending, reasons
  )
  SELECT
    c.tenant_id, c.user_id, c.overall_risk_calc, c.inactivity_risk_calc,
    c.adherence_risk_calc, 0,
    ...
    END,
    false, c.days_inactive, c.current_streak, c.adherence_7d_calc, NULL,
    v_calculated_at, c.operational_status_calc, c.attention_bucket_calc,
    c.last_activity_date, c.eligible_days, c.next_appointment_at,
    c.last_appointment_at, c.active_protocol_end_date,
    c.checkin_overdue_calc, c.consultation_overdue,
    c.plan_expiring_calc, c.protocol_ending_calc,

...
-- Mantém appointment_communication_jobs como fila única e adiciona roteamento,
-- consentimento, fallback, auditoria e claims service-only para o adapter Edge.
...
create policy "Staff manages appointment channel settings"
on public.appointment_communication_channel_settings
for all to authenticated
using (exists (
  select 1 from public.profiles p
  where p.user_id=auth.uid() and p.tenant_id=appointment_communication_channel_settings.tenant_id
...
insert into public.appointment_communication_channel_settings(tenant_id)
...
create policy "Patients read own communication consent"
on public.appointment_communication_consents for select to authenticated
using (patient_id=auth.uid());

drop policy if exists "Staff manages communication consent" on public.appointment_communication_consents;
create policy "Staff manages communication consent"
on public.appointment_communication_consents for all to authenticated
using (exists (
  select 1 from public.profiles p where p.user_id=auth.uid() and p.tenant_id=appointment_communication_consents.tenant_id
    and lower(coalesce(p.role,'')) in ('admin','nutritionist','nutri')
))
with check (exists (
  select 1 from public.profiles p where p.user_id=auth.uid() and p.tenant_id=appointment_communication_consents.tenant_id
...
insert into public.appointment_communication_templates(
  tenant_id,kind,channel,title,body,active,provider_language,provider_parameters
)
select t.id,'confirmation_request','whatsapp','Confirmação de consulta',
       'Template aprovado pela Meta para confirmação de consulta.',false,'pt_BR',
       '["patient_name","appointment_date","appointment_time","appointment_type"]'::jsonb
from public.tenants t
where not exists (
  select 1 from public.appointment_communication_templates x
  where x.tenant_id=t.id and x.kind='confirmation_request' and x.channel='whatsapp'
);

insert into public.appointment_communication_templates(
  tenant_id,kind,channel,title,body,active,provider_language,provider_parameters
)
select t.id,'reminder','whatsapp','Lembrete de consulta',
       'Template aprovado pela Meta para lembrete de consulta.',false,'pt_BR',
       '["patient_name","appointment_date","appointment_time","appointment_type"]'::jsonb
from public.tenants t
where not exists (
  select 1 from public.appointment_communication_templates x
  where x.tenant_id=t.id and x.kind='reminder' and x.channel='whatsapp'
);
...
create policy "Staff reads appointment delivery events"
on public.appointment_communication_delivery_events for select to authenticated
using (exists (
  select 1 from public.profiles p where p.user_id=auth.uid() and p.tenant_id=appointment_communication_delivery_events.tenant_id
    and lower(coalesce(p.role,'')) in ('admin','nutritionist','nutri')
));
...
-- Decide se um job deve ser reservado ao WhatsApp. Falta de consentimento/configuração
-- deixa o Inbox como canal efetivo, sem bloquear a comunicação transacional existente.
create or replace function public.appointment_whatsapp_route_available(
  p_tenant_id uuid,
  p_patient_id uuid,
  p_kind text
) returns boolean
...
-- Inbox continua como fallback e canal padrão. Jobs explicitamente roteados para Inbox
-- sempre são elegíveis, mesmo que o WhatsApp tenha sido habilitado depois.
...
declare
  ...
  v_message_id uuid;
  v_sent integer:=0; v_cancelled integer:=0; v_failed integer:=0;
begin
  ...
  for v_job in
    select j.* from public.appointment_communication_jobs j
    where j.status in ('ready','failed') and j.due_at<=now()
      and (j.next_attempt_at is null or j.next_attempt_at<=now())
      and j.attempt_count<j.max_attempts
      and (
        j.channel='inbox'
        or not public.appointment_whatsapp_route_available(j.tenant_id,j.patient_id,j.kind)
      )
    order by j.due_at,j.created_at for update skip locked limit p_limit
  loop
    begin
      ...
      if v_appointment.id is null or v_appointment.status not in ('scheduled','confirmed','in_progress') then
        update public.appointment_communication_jobs set status='cancelled',locked_at=null,updated_at=now(),last_error=null where id=v_job.id;
      ...
      select * into v_template from public.appointment_communication_templates t
       where t.tenant_id=v_job.tenant_id and t.kind=v_job.kind and t.channel='inbox' and t.active=true;
      if v_template.id is null then
        update public.appointment_communication_jobs set status='failed',attempt_count=attempt_count+1,next_attempt_at=now()+interval '6 hours',locked_at=null,last_error='Template Inbox ativo não encontrado',failed_at=now(),updated_at=now() where id=v_job.id;
        v_failed:=v_failed+1; continue;
      end if;
      if v_job.provider='inbox' and v_job.provider_message_id is not null and exists(select 1 from public.inbox_messages im where im.id::text=v_job.provider_message_id) then
        update public.appointment_communication_jobs set status='sent',sent_at=coalesce(sent_at,now()),delivered_at=coalesce(delivered_at,now()),locked_at=null,last_error=null,updated_at=now() where id=v_job.id;
        continue;
      end if;
      ...
      values(v_job.tenant_id,v_job.patient_id,'appointment_automation',v_template.title,v_template.body,
        ...
        jsonb_build_object('appointment_id',v_job.appointment_id,'communication_job_id',v_job.id,'kind',v_job.kind,'fallback_from',case when v_job.metadata ? 'whatsapp_fallback' then 'whatsapp' else null end))
      returning id into v_message_id;
      update public.appointment_communication_jobs set status='sent',channel='inbox',provider='inbox',provider_message_id=v_message_id::text,
        attempt_count=attempt_count+1,sent_at=now(),delivered_at=now(),failed_at=null,next_attempt_at=null,locked_at=null,last_error=null,updated_at=now()
      where id=v_job.id;
      if v_job.kind='confirmation_request' then update public.appointments set confirmation_sent=true,updated_at=now() where id=v_job.appointment_id;
      elsif v_job.kind='reminder' then update public.appointments set reminder_sent=true,reminder_sent_at=now(),updated_at=now() where id=v_job.appointment_id; end if;
      v_sent:=v_sent+1;
    exception when others then
      update public.appointment_communication_jobs set status='failed',attempt_count=attempt_count+1,
        next_attempt_at=case when attempt_count+1<max_attempts then now()+interval '1 hour' else null end,
        failed_at=now(),locked_at=null,last_error=left(sqlerrm,500),updated_at=now() where id=v_job.id;
      v_failed:=v_failed+1;
    end;
  end loop;
...
-- Claim seguro para a Edge Function. Também aplica quiet hours antes de liberar o job.
...
begin
  ...
  with candidates as (
      ...
      and (
        ...
        or case
          when s.quiet_hours_start < s.quiet_hours_end then
            ...
            not (((now() at time zone coalesce((select tas.timezone from public.tenant_appointment_settings tas where tas.tenant_id=j.tenant_id),'America/Sao_Paulo'))::time >= s.quiet_hours_start)
    ...
    update public.appointment_communication_jobs j
       set locked_at=now(),channel='whatsapp',provider='meta_cloud',updated_at=now()
      from candidates c where j.id=c.id
    returning j.*
  )
  select l.id,l.tenant_id,l.appointment_id,l.patient_id,l.kind,
         regexp_replace(coalesce(p.phone,''),'\\D','','g') as phone,
         coalesce(nullif(p.name,''),'Paciente') as patient_name,
         a.scheduled_at,
         coalesce(tas.timezone,'America/Sao_Paulo') as timezone,
         coalesce(at.name,'Consulta') as appointment_type,
         tpl.provider_template_name,
         coalesce(nullif(tpl.provider_language,''),'pt_BR'),
         coalesce(tpl.provider_parameters,'[]'::jsonb),
         s.whatsapp_phone_number_id,s.whatsapp_graph_version,
         s.whatsapp_access_token_env,s.whatsapp_verify_token_env,s.whatsapp_app_secret_env,
         s.fallback_to_inbox
  from locked l
...
create or replace function public.service_complete_appointment_whatsapp_job(
  p_job_id uuid,
  p_success boolean,
...
set search_path=public,pg_temp
as $$
declare v_job public.appointment_communication_jobs%rowtype; v_fallback boolean:=true; v_attempt integer;
begin
  if current_user not in ('postgres','service_role','supabase_admin') then raise exception 'privileged role required'; end if;
  select * into v_job from public.appointment_communication_jobs where id=p_job_id for update;
  if v_job.id is null then raise exception 'job not found'; end if;
  if v_job.provider<>'meta_cloud' or v_job.channel<>'whatsapp' then raise exception 'job is not a WhatsApp Meta claim'; end if;
  select coalesce(s.fallback_to_inbox,true) into v_fallback from public.appointment_communication_channel_settings s where s.tenant_id=v_job.tenant_id;
  ...
  if p_success then
    ...
    update public.appointment_communication_jobs set status='sent',provider_message_id=p_provider_message_id,attempt_count=v_attempt,
      sent_at=coalesce(sent_at,now()),failed_at=null,next_attempt_at=null,locked_at=null,last_error=null,
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('meta_response',coalesce(p_response_metadata,'{}'::jsonb)),updated_at=now()
    where id=p_job_id;
    insert into public.appointment_communication_delivery_events(tenant_id,job_id,provider,provider_message_id,status,metadata)
      values(v_job.tenant_id,p_job_id,'meta_cloud',p_provider_message_id,'accepted',coalesce(p_response_metadata,'{}'::jsonb));
    if v_job.kind='confirmation_request' then update public.appointments set confirmation_sent=true,updated_at=now() where id=v_job.appointment_id;
    elsif v_job.kind='reminder' then update public.appointments set reminder_sent=true,reminder_sent_at=now(),updated_at=now() where id=v_job.appointment_id; end if;
    return jsonb_build_object('status','sent','attempt',v_attempt);
  end if;

  if p_retryable and v_attempt<v_job.max_attempts then
    update public.appointment_communication_jobs set status='failed',attempt_count=v_attempt,failed_at=now(),locked_at=null,
      next_attempt_at=now()+case v_attempt when 1 then interval '5 minutes' when 2 then interval '30 minutes' else interval '2 hours' end,
      last_error=left(coalesce(p_error,'WhatsApp provider error'),500),updated_at=now() where id=p_job_id;
    return jsonb_build_object('status','retry','attempt',v_attempt);
  end if;

  if v_fallback then
    update public.appointment_communication_jobs set status='ready',channel='inbox',provider=null,provider_message_id=null,
      attempt_count=0,failed_at=now(),next_attempt_at=null,locked_at=null,last_error=left(coalesce(p_error,'WhatsApp provider error'),500),
      metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('whatsapp_fallback',jsonb_build_object('at',now(),'attempts',v_attempt,'reason',left(coalesce(p_error,'provider_error'),300))),updated_at=now()
    where id=p_job_id;
    return jsonb_build_object('status','fallback_inbox','attempts',v_attempt);
  end if;

  update public.appointment_communication_jobs set status='failed',attempt_count=v_attempt,failed_at=now(),next_attempt_at=null,locked_at=null,
    last_error=left(coalesce(p_error,'WhatsApp provider error'),500),updated_at=now() where id=p_job_id;
  return jsonb_build_object('status','failed','attempts',v_attempt);
end;
$$;
...
-- Token interno do cron -> Edge. Apenas o hash fica em tabela pública sem grants; plaintext fica no Vault.
create table if not exists public.internal_dispatch_tokens (
...
begin
  if not exists(select 1 from public.internal_dispatch_tokens where name='appointment_whatsapp_dispatch') then
    ...
    insert into public.internal_dispatch_tokens(name,token_hash) values('appointment_whatsapp_dispatch',encode(digest(v_token,'sha256'),'hex'));
...
create or replace function public.service_verify_appointment_dispatch_token(p_token text)
returns boolean language sql stable security definer
set search_path=public,pg_temp
as $$ select coalesce(encode(digest(coalesce(p_token,''),'sha256'),'hex')=(select token_hash from public.internal_dispatch_tokens where name='appointment_whatsapp_dispatch'),false) $$;
...
-- Agenda o adapter Edge sem consumir cron da Vercel. O endpoint valida x-dispatch-token.
do $$
...
begin
  ...
  perform cron.schedule(
    'appointment-whatsapp-meta-dispatch',
    '*/5 * * * *',
    $cron$
      select net.http_post(
        ...
        headers:=jsonb_build_object(
          ...
          'x-dispatch-token',(select decrypted_secret from vault.decrypted_secrets where name='appointment_whatsapp_dispatch_token' order by created_at desc limit 1)
        ),
